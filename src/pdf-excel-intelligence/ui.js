...
async function scan(){
  ...
  }catch(e){status.className='warn';status.textContent=`Lỗi: ${e?.message||e}`;}
}
...
async function exportXlsx(){
  ...
  try{const info=await buildInfo();await exportPdfExcelWorkbook({docs:latest.docs,result:latest,question:panel.querySelector('#p4q').value,imageMode:panel.querySelector('#p4img').value,includeText:true,includeFormulas:true,sourceSha:String(info.commit||'')});status.className='ok';status.textContent='Đã xuất Excel REVIEW. OCR/Vision và ứng viên PDF chưa được tự nâng thành VERIFIED.';}catch(e){status.className='warn';status.textContent=`Xuất Excel lỗi: ${e?.message||e}`;}finally{btn.disabled=false;}
}
