...
export const TEXT_INDEX_VERSION = 3;
...
function joinTextRow(row) {
  ...
  for (const item of items) {
    ...
    // PDF.js may emit one glyph or one syllable per text item. Inserting a
    // space unconditionally turns “Cọc chống” into “C ọ c c h ố n g”, which
    // breaks exact Vietnamese retrieval. Geometry is a better word separator.
...
export async function parsePdf(file, onProgress = () => {}) {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    pages.push({ page: i, text, textQuality: textQuality(text) });
  ...
  const doc = {
    ...
    scannedLikely: totalChars < Math.max(120, pdf.numPages * 35) || pages.filter(p => p.textQuality?.usable).length < Math.max(2, Math.ceil(pdf.numPages * 0.35)),
...
/**
 * Rebuild the searchable text index from the original PDF blob.
 * This migrates documents already stored in IndexedDB by older HNL versions,
 * whose text rows may have been joined with spaces between every PDF.js item.
 */
export async function reindexPdfText(doc, onProgress = () => {}) {
  if (!doc?.blob || doc.viewerKind !== 'pdf') return { doc, changed:false, reason:'not-pdf' };
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    totalChars += text.length;
    pages.push({ page:i, text, textQuality:quality });
    onProgress(i, pdf.numPages);
  }
  doc.pages = pages;
  doc.pageCount = pdf.numPages;
  doc.textChars = totalChars;
  doc.scannedLikely = totalChars < Math.max(120, pdf.numPages * 35) || pages.filter(p => p.textQuality?.usable).length < Math.max(2, Math.ceil(pdf.numPages * 0.35));
  doc.textIndexVersion = TEXT_INDEX_VERSION;
  doc.textReindexedAt = new Date().toISOString();
