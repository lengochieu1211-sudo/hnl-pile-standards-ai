...
import { createP4ReviewConfirmation } from './confirmation-contract.js';
import { createP4ReviewedNativeExportPacket, exportP4ReviewedProductionWorkbook } from './review-production-export.js';
...
function nativeReviewPrompt(source={}){
  const name=String(source.standard||source.docName||'PDF đang mở'),page=Number(source.page)||1;
  return `XÁC NHẬN XUẤT EXCEL PRODUCTION-REVIEWED\n\nNguồn: ${name}\nTrang: ${page}\n\nTôi đã đối chiếu trực tiếp nội dung trên PDF native và xác nhận vùng/giá trị đang chọn đúng với tài liệu nguồn.\n\nXác nhận này CHỈ cho phép xuất workbook. Không đưa dữ liệu vào Calculation Engine và không nâng OCR/Vision thành VERIFIED.\n\nTiếp tục?`;
}
async function onExportReviewedNative(source){
  if(String(source?.method||'')!=='text-layer')return notice('Production-reviewed chỉ cho PDF native text-layer. OCR/Vision/scan vẫn chỉ được xuất REVIEW.','error');
  if(!String(source?.text||'').trim())return notice('Vùng PDF native chưa có nội dung để xác nhận.','error');
  if(!window.confirm(nativeReviewPrompt(source)))return;
  try{
    notice('Đang tạo Excel Production-reviewed từ PDF native…');
    const info=await buildInfo(),packet=p4PacketFromSelectionSource(source,info),fieldKey='__packet__';
    const confirmation=createP4ReviewConfirmation({source:packet.source,fieldKey,value:packet.text,confirmedBy:'user',note:'Người dùng đã đối chiếu trực tiếp PDF native và xác nhận cho phạm vi xuất workbook.'});
    const reviewed=createP4ReviewedNativeExportPacket(packet,confirmation,{fieldKey,value:packet.text});
    const name=`HNL_P4_PRODUCTION_REVIEWED_${safeFilePart(source.standard||source.docName)}_P${Number(source.page)||1}_${new Date().toISOString().slice(0,19).replace(/[:T]/g,'-')}.xlsx`;
    const outcome=await exportP4ReviewedProductionWorkbook(reviewed,confirmation,{fieldKey,value:reviewed.text,fileName:name,validateOnly:false});
    emitRuntime('hnl:p4-runtime-export',{kind:'pdf-native-production-reviewed-xlsx',ok:outcome?.ok!==false,fileName:name,build:buildSummary(info),source:reviewed.source,reviewProduction:reviewed.reviewProduction,safety:safety()});
    notice('Đã xuất Excel Production-reviewed. Calculation Engine vẫn khóa.','success');
  }catch(e){notice(`Production-reviewed bị chặn: ${e?.message||e}`,'error');emitRuntime('hnl:p4-runtime-error',{kind:'pdf-native-production-reviewed-xlsx',message:String(e?.message||e)});}
}

function enhancePopup(popup){if(!popup||popup.dataset.p4Excel==='1')return;const actions=popup.querySelector('.pdf-selection-actions');if(!actions)return;const b=document.createElement('button');b.type='button';b.textContent='Xuất Excel thông minh';b.dataset.p4UiAction='excel';b.title='P4 · vùng PDF → bảng/công thức/tham số/provenance → Excel REVIEW-first';actions.appendChild(b);const p=document.createElement('button');p.type='button';p.textContent='Xuất Production đã đối chiếu';p.dataset.p4UiAction='production';p.title='Chỉ PDF native text-layer; yêu cầu xác nhận trực tiếp nguồn. Không gọi Calculation Engine.';actions.appendChild(p);popup.dataset.p4Excel='1';}
function enhanceToolbar(){const anchor=document.querySelector('#pdfSmartSelect');if(!anchor||document.querySelector('#p4ExportCurrentPage'))return;const review=document.createElement('button');review.type='button';review.className='icon-btn';review.id='p4ExportCurrentPage';review.textContent='XL';review.title='P4 · Xuất trang/vùng PDF hiện tại sang Excel REVIEW';anchor.insertAdjacentElement('afterend',review);const prod=document.createElement('button');prod.type='button';prod.className='icon-btn';prod.id='p4ExportCurrentPageProduction';prod.textContent='PX';prod.title='P4 · PDF native đã đối chiếu → Excel Production-reviewed; Calculation Engine vẫn khóa';review.insertAdjacentElement('afterend',prod);}
...
function install(){
  ...
  document.addEventListener('click',async e=>{const excelBtn=e.target?.closest?.('[data-p4-ui-action="excel"]');if(excelBtn){e.preventDefault();const popup=excelBtn.closest('.pdf-selection-popup'),source=popup?._hnlSource;if(source)await onExportSelection(source);return;}const productionBtn=e.target?.closest?.('[data-p4-ui-action="production"]');if(productionBtn){e.preventDefault();const popup=productionBtn.closest('.pdf-selection-popup'),source=popup?._hnlSource;if(source)await onExportReviewedNative(source);return;}const imgBtn=e.target?.closest?.('[data-p4-image-action="excel"]');if(imgBtn){e.preventDefault();try{notice('Đang tạo Excel REVIEW từ ảnh…');const r=await exportImageReview(imgBtn.closest('.image-engineering-review'));emitRuntime('hnl:p4-runtime-export',{kind:'image-review-xlsx',ok:r.outcome?.ok!==false,fileName:r.name,build:buildSummary(r.info),source:r.packet.source,trust:r.packet.trust,parameterCount:r.packet.parameters?.length||0,figureCount:r.packet.figures?.length||0,safety:safety()});notice('Đã xuất Excel REVIEW từ ảnh.','success');}catch(err){notice(`Không xuất được Excel ảnh: ${err?.message||err}`,'error');emitRuntime('hnl:p4-runtime-error',{kind:'image-review-xlsx',message:String(err?.message||err)});}return;}if(e.target?.closest?.('#p4ExportCurrentPageProduction')){e.preventDefault();const source=sourceFromCurrentTextLayer();if(!source)return notice('Trang hiện tại chưa có PDF native text-layer. Production-reviewed không dùng OCR/Vision.','error');await onExportReviewedNative(source);return;}if(e.target?.closest?.('#p4ExportCurrentPage')){e.preventDefault();const source=sourceFromCurrentTextLayer();if(!source)return notice('Trang hiện tại chưa có text-layer. Hãy kéo vùng OCR/Vision rồi chọn “Xuất Excel thông minh”.','error');await onExportSelection(source);}},true);
  ...
  window.__HNL_P4_PDF_EXCEL__={open,scan:()=>{open();return scan();},exportReviewedNative:onExportReviewedNative};
}
